...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo project-hardening
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/project-hardening/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/food-ordering-system-ms-project-hardening-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/food-ordering-system-ms-project-hardening"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/project-hardening/executar-tarefas.sh --faixa $1"
...
T-001 — "Tornar build, CI e repositório reproduzíveis"
  critérios/refs: AC-001 (Build completo funciona pelo Maven Wrapper), AC-002 (Integração contínua valida cada mudança), AC-003 (Arquivos locais e gerados ficam fora do versionamento)
  arquivos permitidos (e seus testes): .mvn/wrapper/maven-wrapper.properties, mvnw, mvnw.cmd, .gitignore, .github/workflows/ci.yml, quality-tests/build-foundation.test.mjs, onpspec.config.json
  mensagem de commit: "T-001 project-hardening: Tornar build, CI e repositório reproduzíveis"
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/project-hardening/executar-tarefas.sh --gate"
      exit 0
    fi
